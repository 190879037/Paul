    ...
    foreach ($hMon in (Get-AllHMonitors)) {
      ...
      try {
        for ($i=0; $i -lt $n; $i++) {
          ...
          if (-not $ok) { Start-Sleep -Milliseconds 120; [void][ClearyNative]::SetMonitorBrightness($ph,[uint32]$v) }
    ...
    foreach ($hMon in (Get-AllHMonitors)) {
      ...
      try {
        for ($i=0; $i -lt $n; $i++) {
          ...
          if (-not $ok) { Start-Sleep -Milliseconds 120; [void][ClearyNative]::SetMonitorContrast($ph,[uint32]$v) }
          Start-Sleep -Milliseconds 40
          $min=0;$cur=0;$max=0
