...
$script:I18n = @{
  zh = [ordered]@{
    ...
    themeTitle = '外观主题'
    themeHintPick = '选择深色、浅色，或跟随 Windows 系统。'
    themeHintLight = '当前界面：浅色（云母白）'
    themeHintDark = '当前界面：深色'
    themeHintSystemFmt = '当前界面：跟随系统（当前 Windows 为{0}）'
    themeLightShort = '浅'
    themeLight = '浅色'
    themeLightDesc = '云母白界面，适合白天与高亮度环境'
    themeDarkShort = '深'
    themeDark = '深色'
    themeDarkDesc = '深色云母风格，夜间更护眼'
    themeSysShort = '自'
    themeSystem = '跟随系统'
  ...
  en = [ordered]@{
    ...
    themeHintPick = 'Choose light, dark, or follow Windows.'
    themeHintLight = 'UI theme: Light (mica white)'
    themeHintDark = 'UI theme: Dark'
    themeHintSystemFmt = 'UI theme: System (Windows is {0})'
    themeLightShort = 'L'
    themeLight = 'Light'
    themeLightDesc = 'Mica-white UI for daytime and bright rooms'
    themeDarkShort = 'D'
    themeDark = 'Dark'
    themeDarkDesc = 'Dark mica style, easier at night'
    themeSysShort = 'A'
    themeSystem = 'System'
...
function Apply-Recommend([string]$id, [bool]$persist = $true) {
  ...
  if (-not $script:uiSettings) { $script:uiSettings = @{ theme = 'light'; recommend = $id } }
...
function Apply-UiTheme([string]$mode) {
  try {
    ...
    if ($mode -notin @('light','dark','system')) { $mode = 'light' }
    $script:uiSettings.theme = $mode
    $effectiveDark = $false
    if ($mode -eq 'dark') { $effectiveDark = $true }
    elseif ($mode -eq 'system') { $effectiveDark = -not (Get-SystemIsLight) }

    if ($effectiveDark) {
      Set-ResourceBrush 'WinBg'        '#1C1C1E'
      Set-ResourceBrush 'CardBg'       '#2C2C2E'
      Set-ResourceBrush 'CardBorder'   '#3A3A3C'
      Set-ResourceBrush 'TextPrimary'  '#F2F2F7'
      Set-ResourceBrush 'TextSecondary' '#A1A1A6'
      Set-ResourceBrush 'TextMuted'    '#8E8E93'
      Set-ResourceBrush 'Accent'       '#0A84FF'
      Set-ResourceBrush 'AccentSoft'   '#1A3A5C'
      Set-ResourceBrush 'GhostBg'      '#3A3A3C'
      Set-ResourceBrush 'TrackBg'      '#636366'
      Set-ResourceBrush 'InputBg'      '#1C1C1E'
      Set-ResourceBrush 'InputBorder'  '#48484A'
      Set-ResourceBrush 'ThumbFill'    '#2C2C2E'
    } else {
      Set-ResourceBrush 'WinBg'        '#F7F8FA'
      Set-ResourceBrush 'CardBg'       '#FFFFFF'
      Set-ResourceBrush 'CardBorder'   '#E8EAEF'
      Set-ResourceBrush 'TextPrimary'  '#1A1A1A'
      Set-ResourceBrush 'TextSecondary' '#5C5C5C'
      Set-ResourceBrush 'TextMuted'    '#8A8A8A'
      Set-ResourceBrush 'Accent'       '#0078D4'
      Set-ResourceBrush 'AccentSoft'   '#EEF6FF'
      Set-ResourceBrush 'GhostBg'      '#F0F2F5'
      Set-ResourceBrush 'TrackBg'      '#B8BCC4'
      Set-ResourceBrush 'InputBg'      '#FFFFFF'
      Set-ResourceBrush 'InputBorder'  '#C8CCD4'
      Set-ResourceBrush 'ThumbFill'    '#FFFFFF'
    }

    if ($window) { $window.Background = Get-ResourceBrush 'WinBg' }
    if ($TxtThemeHint) {
      if ($mode -eq 'light') { $TxtThemeHint.Text = (T 'themeHintLight') }
      elseif ($mode -eq 'dark') { $TxtThemeHint.Text = (T 'themeHintDark') }
      else {
...
function Apply-UiLanguage([string]$lang, [bool]$persist = $true) {
  ...
  if ($script:uiSettings.theme) {
    ...
    if ($TxtThemeHint) {
      if ($m -eq 'light') { $TxtThemeHint.Text = (T 'themeHintLight') }
      elseif ($m -eq 'dark') { $TxtThemeHint.Text = (T 'themeHintDark') }
      else {
        $sys = if (Get-SystemIsLight) { T 'lightWord' } else { T 'darkWord' }
        $TxtThemeHint.Text = (TF 'themeHintSystemFmt' $sys)
      }
...
if (-not $script:uiSettings -or -not $script:uiSettings.theme) {
  $script:uiSettings = @{ theme = 'light'; recommend = 'generic'; lang = 'zh' }
}
  ...
  <Grid Margin="20">
          ...
          <StackPanel>
            <Border Style="{StaticResource Card}">
              <StackPanel>
                ...
                <TextBlock x:Name="TxtThemeHint" Text="选择深色、浅色，或跟随 Windows 系统。" Foreground="{DynamicResource TextSecondary}" TextWrapping="Wrap" Margin="0,4,0,16"/>

                <Border x:Name="BdThemeLight" CornerRadius="12" BorderThickness="2" Padding="16" Margin="0,0,0,10" Cursor="Hand"
                  ...
                  <Grid>
                    ...
                    <Border Width="40" Height="40" CornerRadius="20" Background="#F7F8FA" BorderBrush="#E8EAEF" BorderThickness="1" Margin="0,0,14,0">
                      <TextBlock x:Name="TxtThemeLightShort" Text="浅" FontSize="16" FontWeight="Light" HorizontalAlignment="Center" VerticalAlignment="Center" Foreground="#1A1A1A"/>
                    </Border>
                    <StackPanel Grid.Column="1" VerticalAlignment="Center">
                      <TextBlock x:Name="TxtThemeLightName" Text="浅色" FontSize="15" FontWeight="SemiBold"/>
                      <TextBlock x:Name="TxtThemeLightDesc" Text="云母白界面，适合白天与高亮度环境" Foreground="{DynamicResource TextSecondary}" Margin="0,2,0,0"/>
                    </StackPanel>
                ...
                <Border x:Name="BdThemeDark" CornerRadius="12" BorderThickness="1" Padding="16" Margin="0,0,0,10" Cursor="Hand"
                  ...
                  <Grid>
                    ...
                    <Border Width="40" Height="40" CornerRadius="20" Background="#2C2C2E" BorderBrush="#3A3A3C" BorderThickness="1" Margin="0,0,14,0">
                      <TextBlock x:Name="TxtThemeDarkShort" Text="深" FontSize="16" FontWeight="Light" HorizontalAlignment="Center" VerticalAlignment="Center" Foreground="#F2F2F7"/>
                    </Border>
                    <StackPanel Grid.Column="1" VerticalAlignment="Center">
                      <TextBlock x:Name="TxtThemeDarkName" Text="深色" FontSize="15" FontWeight="SemiBold"/>
                      <TextBlock x:Name="TxtThemeDarkDesc" Text="深色云母风格，夜间更护眼" Foreground="{DynamicResource TextSecondary}" Margin="0,2,0,0"/>
                    </StackPanel>
                ...
                <Border x:Name="BdThemeSystem" CornerRadius="12" BorderThickness="1" Padding="16" Cursor="Hand"
                  ...
                  <Grid>
                    ...
                    <Border Width="40" Height="40" CornerRadius="20" Background="#EEF6FF" BorderBrush="#0078D4" BorderThickness="1" Margin="0,0,14,0">
                      <TextBlock x:Name="TxtThemeSysShort" Text="自" FontSize="16" FontWeight="Light" HorizontalAlignment="Center" VerticalAlignment="Center" Foreground="{DynamicResource Accent}"/>
                    </Border>
                    <StackPanel Grid.Column="1" VerticalAlignment="Center">
    ...
    <Border Grid.Row="2" Margin="0,8,0,0" Padding="4,10,4,0">
      <Grid>
        ...
        <TextBlock x:Name="TxtFooterHint" VerticalAlignment="Center" Foreground="{DynamicResource TextMuted}" FontSize="12"
